...
out <- pmc(phy, dat, "BM", "lambda", nboot = 50, mc.cores = cores)
```
...
bm_v_lambda <- pmc(geospiza$phy, geospiza$dat[, "wingL"],
  "BM", "lambda", nboot=100, mc.cores = cores) 
...
ou3v4 <- pmc(tree, log(anoles["size"]), modelA = "hansen", modelB = "hansen", 
             ...
             nboot = 100, sqrt.alpha = 1, sigma = 1, mc.cores = cores)
